...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($contactNumber) || !preg_match('/^[0-9]{10}$/', $contactNumber)) {
    ...
    if (empty($errors) && $paymentMethod !== 'khaltiPayment') {
        try {
            ...
            foreach ($cartItems as $item) {
                $price = $item['discount_price'] ?: $item['price'];
                executeQuery("INSERT INTO order_items (order_id, product_id, quantity, unit_price)
    ...
    // 🟣 KHALTI FLOW
    elseif (empty($errors) && $paymentMethod === 'khaltiPayment') {
        $row = executeQuery("SELECT full_name, email FROM users WHERE user_id = ?", [$userId])->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $name = $row['full_name'];
            $email = $row['email'];
        ...
        $amount = $total * 100; // Khalti expects amount in paisa
        $purchase_order_id = rand(100000, 999999); // You can generate dynamically
        $purchase_order_name = "gaming_hub";
        $_SESSION['name'] = $name;

        $postFields = array(
            "return_url" => "http://localhost/gaming_hub/verify_khalti_payment.php",
            "website_url" => "http://localhost/gaming_hub/",
            "amount" => $amount,
        ...
        if (curl_errno($curl)) {
            ...
            if (isset($responseArray['error'])) {
                $errors[] = 'Khalti Error: ' . $responseArray['error'];
            } elseif (isset($responseArray['payment_url'])) {
                header('Location: ' . $responseArray['payment_url']);
                exit();
            } else {
                $errors[] = 'Unexpected response from Khalti: ' . $response;
            }
        }

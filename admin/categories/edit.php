...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $slugCheck = executeQuery(
        "SELECT category_id FROM main_categories WHERE slug = ? AND category_id != ?",
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        $upload = uploadFile($_FILES['image'], __DIR__ . '../../../assets/uploads/categories/');
        if ($upload['success']) {
            // Delete old image if it exists
            if ($currentImage && file_exists(__DIR__ . '../../../assets/uploads/categories/' . $currentImage)) {
                unlink(__DIR__ . '../../../assets/uploads/categories/' . $currentImage);
            }
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE main_categories SET name = ?, slug = ?, image = ?, is_active = ? WHERE category_id = ?";
            executeQuery($sql, [
                $category['name'],
                $category['slug'],
                $currentImage,
            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6>Edit Category: <?= htmlspecialchars($category['name']) ?></h6>
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    ...
                    <form method="POST" enctype="multipart/form-data" class="p-4">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <?php if ($currentImage): ?>
                                <div class="mb-2">
                                    <img src="../../assets/uploads/categories/<?= $currentImage ?>" alt="Current Image"

...
$subcategory = executeQuery("SELECT * FROM subcategories WHERE subcategory_id = ?", [$subcategoryId])
...
$category = executeQuery("SELECT name FROM main_categories WHERE category_id = ?", [$categoryId])
            ->fetch(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $slugCheck = executeQuery("SELECT subcategory_id FROM subcategories WHERE slug = ? AND category_id = ? AND subcategory_id != ?", 
                             [$subcategory['slug'], $categoryId, $subcategoryId])->fetch();
    if ($slugCheck) {
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE subcategories SET name = ?, slug = ?, is_active = ? WHERE subcategory_id = ?";
            executeQuery($sql, [
                $subcategory['name'],
                $subcategory['slug'],
            ...
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6>Edit Subcategory for <?= htmlspecialchars($category['name']) ?></h6>
